...
// Plus/Minus 10 and 100 — mentally add or subtract 10 or 100 from a 3-digit
// number. Rocket/spaceship theme — the player navigates through space by
// counting by 10s and 100s.
//   🌱 Easy   (±10 within 100)
//   🌿 Medium (±10 and ±100 within 200)
//   🌳 Hard   (±10 or ±100 with any 3-digit starting number)
// CCSS 2.NBT.B.8: Mentally add 10 or 100 to a given number 100–900, and
...
function makeQuestion(difficulty: Difficulty): Question {
  let start: number, delta: number;
  const ops = [10, -10, 100, -100];
  delta = pick(ops);

  if (difficulty === 0) {
    // Easy: within 100, only ±10
    start = randInt(11, 89);
    delta = delta === 100 || delta === -100 ? (delta > 0 ? 10 : -10) : delta;
  } else if (difficulty === 1) {
    // Medium: within 200, ±10 or ±100 (constrained)
    start = randInt(105, 195);
  } else {
    // Hard: 3-digit, any ±10 or ±100
    if (delta > 0) {
      start = randInt(100, 900 - delta);
    } else {
      start = randInt(100 - delta, 900);
    }
  }

  const correct = start + delta;
  const choices = shuffle([correct, correct + 1, correct - 1, correct + 10, correct - 10, correct + delta]).slice(0, 4);
  const opLabel = delta === 10 ? '+ 10' : delta === -10 ? '− 10' : delta === 100 ? '+ 100' : '− 100';
  return {
    prompt: `What is ${start} ${opLabel}?`,
    start,
    delta,
    correct,
    choices,
...
export default function PlusMinus10And100({ onBack, kidName }: { onBack: () => void; kidName?: string }) {
  ...
  useEffect(() => {
    try {
      const s = localStorage.getItem('plusminus10and100_best_streak');
  ...
  const answer = useCallback((choice: number) => {
    ...
    if (isCorrect) {
      ...
      setLocked(true);
      setFlash('good');
      setFeedback({ kind: 'good', text: `✅ ${question.start} ${question.delta > 0 ? '+' : '−'} ${Math.abs(question.delta)} = ${question.correct}` });
      const isLast = newCurrent >= TOTAL_ROUNDS;
      setTimeout(() => {
        setFlash(null);
        try {
          if (newStreak > bestStreak) {
            localStorage.setItem('plusminus10and100_best_streak', String(newStreak));
      ...
      setStreak(0);
      setFlash('bad');
      setFeedback({ kind: 'bad', text: `Not quite. ${question.start} ${question.delta > 0 ? '+' : '−'} ${Math.abs(question.delta)} = ${question.correct}.` });
    ...
    return (
      <div className="canvas-page slide-up" style={{ textAlign: 'center', maxWidth: 560 }}>
        ...
        <p style={{ marginTop: 16, fontSize: 13, color: 'var(--text-medium)' }}>
          10 launches per mission. 3… 2… 1… liftoff!
        </p>
      </div>
    );
  }
  ...
  if (screen === 'results') {
    ...
    return (
      <div className="canvas-page slide-up" style={{ textAlign: 'center' }}>
        ...
        {showRating && !rated && (
          <RatingModal
            activity="plus-minus-10-and-100"
            activityName="Plus/Minus 10 and 100"
            activityEmoji="🚀"
            kidName={kidName || ''}
  ...
  return (
    ...
    >
      ...
      <h1 className="page-title" style={{ marginBottom: 6 }}>🚀 Plus/Minus 10 and 100</h1>
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: 18, color: 'var(--text-dark)' }}>
          {question.prompt}
        </p>
        <div style={{ marginTop: 14, fontFamily: 'Fredoka, sans-serif', fontSize: 56, fontWeight: 700, color: 'var(--accent-purple)' }}>
          {question.start}
          <span style={{ margin: '0 12px', opacity: 0.6 }}>{question.delta > 0 ? '+' : '−'}</span>
          <span style={{ color: question.delta >= 100 ? 'var(--accent-orange)' : 'var(--accent-blue)' }}>{Math.abs(question.delta)}</span>
      ...
      <p style={{ textAlign: 'center', marginTop: 14, fontSize: 13, color: 'var(--text-medium)' }}>
        Tip: ±<strong style={{ color: 'var(--accent-blue)' }}>10</strong> changes the tens digit. ±<strong style={{ color: 'var(--accent-orange)' }}>100</strong> changes the hundreds digit. The other digit stays the same!
      </p>
